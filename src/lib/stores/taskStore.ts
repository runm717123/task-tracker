  ...
  async importTasks(
    ...
    try {
      ...
      if (!Array.isArray(jsonData)) {
        return {
          success: false,
          message: "Invalid JSON format: Expected an array of tasks",
          imported: 0,
        };
      }
      ...
      if (validTasks.length === 0) {
        return {
          success: false,
          message: "No valid tasks found in the uploaded file",
          imported: 0,
        };
      }
      ...
      if (newTasks.length === 0) {
        return {
          success: false,
          message: "All tasks in the file already exist",
          imported: 0,
        };
      }
  ...
  async getLatestStartTime(dateString: string | Date): Promise<string> {
    ...
    if (tasksForDate.length > 0) {
      ...
      if (completedTasks.length > 0) {
        const latestEndTime = completedTasks
          .map((task) => task.end!)
        ...
        return targetDate
          .set("hour", latestEndMoment.hour())
          .set("minute", latestEndMoment.minute())
          .set("second", latestEndMoment.second())
          .toISOString();
      }
    }
    ...
    return targetDate
      .set("hour", settingsStartMoment.hour())
      .set("minute", settingsStartMoment.minute())
      .set("second", settingsStartMoment.second())
      .toISOString();
  }
